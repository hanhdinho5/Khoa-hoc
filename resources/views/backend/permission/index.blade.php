                ...
                <div class="col-lg-12">
                    <ul class="nav nav-pills mb-3">
                        <li class="nav-item"><a href="#list-view" data-toggle="tab"
                                class="nav-link btn-primary mr-1 show active">Danh sách quyền</a></li>
                                ...
                                @foreach (Illuminate\Support\Facades\Route::getRoutes() as $v)
                                    @if ($v->getPrefix() == '/admin')
                                        @php
                                            ...
                                            if (isset($rl[1])) {
                                                $routes[$rl[0]][] = ['method' => $v->methods[0], 'function' => $rl[1]];
                                            }
                                        @endphp
                                    @endif
                                @endforeach
                                <form action="{{ route('permission.save', encryptor('encrypt', $role->id)) }}"
                                    method="post">
                                    @csrf
                                    <div class="row p-2">
